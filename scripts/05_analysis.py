...
def main():
    ...
    for period_label in [HIST_LABEL, REC_LABEL]:
        ...
        if "source_water_type" in df_reg.columns:
            swt = df_reg["source_water_type"].astype(str).str.upper().str.strip()
            df_reg["is_groundwater"] = (
                swt.str.contains("GROUND", na=False) | swt.isin(["GW", "GU"])
            ).astype(int)
        else:
            df_reg["is_groundwater"] = 0

        # Build formula — drop is_groundwater if it has no variance (all 0 or all 1)
        gw_vals = df_reg["is_groundwater"].nunique()
        if gw_vals > 1:
            formula = "health_violations ~ log_pop + poverty_rate + pct_nonwhite + is_groundwater"
        else:
            formula = "health_violations ~ log_pop + poverty_rate + pct_nonwhite"
    ...
    if "source_water_type" in df_pool.columns:
        swt = df_pool["source_water_type"].astype(str).str.upper().str.strip()
    ...
    # Build formula — drop is_groundwater if constant
    gw_vals = df_pool["is_groundwater"].nunique()
    if gw_vals > 1:
        pooled_formula = (
